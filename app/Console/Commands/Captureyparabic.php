...
class Captureyparabic extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'yp:capturearabic';
     ...
     * @var string
     */
    protected $description = 'Capture all yellowpages arabic data';
    ...
    function get_companys_in_cat($cat){
        ...
        if(strpos($pagedata,'aria-label="الاخير"')!==false){
            $end_pos=strpos($pagedata,'aria-label="الاخير"');
            $last_page_tag=substr($pagedata,$end_pos-8,$end_pos);
            $last_page_number=Self::get_string_between($last_page_tag,'/p','" aria-label');
            if($last_page_number>245){$last_page_number=1544;}//not original
            }
            echo "\n";
            echo "last page is ".$last_page_number;
        ...
        if (strpos($pagedata, 'لا توجد نتائج مطابقة لهذا البحث') !== false){echo "End of pages\n";return $company_list;}
        ...
        $phones_response = $client->request('GET', 'https://www.yellowpages.com.eg/ar/getPhones/'.$company_id.'/false');
...
$all_companies_data=array();
        ...
        for($i=29;$i<=$cat_pages_num;$i++){

            echo "\n";
            echo "Main categories page is ".$i;
        
        $response = $client->request('GET', 'https://www.yellowpages.com.eg/ar/related-categories/p'.$i);
...
// print_r($all_cat[$i-2]);
        ...
        $page_categories=count($all_cat[$i-29]);    //original
        // $page_categories=2;
        ...
        for($perpage=0;$perpage<$page_categories;$perpage++){
            ...
            // echo "companies in this cat is ".$all_cat[$i-1][$perpage][2];

        $companies_in_category=Self::get_companys_in_cat($all_cat[$i-29][$perpage][0]);  //original
        

foreach ($companies_in_category as $line) {
    $temp_array=array_merge($all_cat[$i-29][$perpage],$line);


    DB::table('yellowpagesarabic')->insert(
        ['url' => $temp_array[0],
        'cat' => $temp_array[1],
